...
// иконки 
import {BsFillBookmarkPlusFill, BsFillBrightnessHighFill} from "react-icons/bs";
// доступ к provider, который находится над всеми
...
//console.log(watchList);
	...
	const renderIcon = (change) => {
		return change > 0 ? <BsFillBrightnessHighFill/>:<BsFillBookmarkPlusFill/>
	}
				...
				{stock.map((stockData) => {
					return (
						<tr style={{cursor: "pointer"}} onClick={()=> handleStockSelect(stockData.symbol)} className="table-row" key={stockData.symbol}>
							...
							<td>{stockData.data.pc} <button className="btn btn-danger btn-sm ml-3 d-inlin-block delete-button"
							 onClick={(e)=>{
							 	e.stopPropagation()
